...
【参考：2024-2025年度の主要補助金の上限額情報（事実データ）】
AIは提案を生成する際、以下の正確な上限額情報を引用し、虚偽の金額を提示しないでください。
- IT導入補助金: 最大450万円（通常枠・インボイス枠） / 複数社連携では最大3000万円
- ものづくり補助金: 1000万〜最大4000万円（大幅賃上げ特例・グローバル枠等）
- 事業再構築補助金: 3000万〜最大1.5億円（成長分野・GX進出類型等）
- 中小企業省力化投資補助金: カタログ型最大1500万円 / 一般型最大1億円
- 小規模事業者持続化補助金: 最大250万円（賃上げ・インボイス特例適用時）
...

{
  ...
  "q5": ["選択したプロパティの番号"],
  "q6": ["選択したプロパティの番号"],
  "aiProposal": "サイトの情報から読み取れる企業の特徴（従業員規模や業種）に基づき、高額な補助金が見込める野心的な事業計画（例：『AIケアプラン自動作成事業』『自社専用の受発注DXシステム』など）を箇条書きで3〜4つ提案してください。その際、必ず参照データ（事実データ）に基づいた【正確な補助金名と獲得可能な上限額】をセットで明記してください。最後に提案内容を合算し、『これらを組み合わせれば、御社の規模感であれば○○万円〜○○万円以上の補助金を狙える可能性があります。』というように、具体的な企業規模に基づいた現実的かつ強気な最大見込み額で締めくくってください。絶対に全て同じ金額（3000万〜1億など）を固定で出力せず、読み取った企業情報に合わせて金額を変動させてください。"
}
`;
